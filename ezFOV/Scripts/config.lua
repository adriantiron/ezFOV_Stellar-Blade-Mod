...
local function sanitize_boolean(v)
    ...
    if type(v) == "string" then
        ...
        if clean == "true" or clean == "1" then return true end
        if clean == "false" or clean == "0" then return false end
    end
...
local function deep_copy_defaults()
    return {
        ...
        DefaultPosition = { x = DEFAULTS.DefaultPosition.x, y = DEFAULTS.DefaultPosition.y, z = DEFAULTS.DefaultPosition.z },
        CombatPosition  = { x = DEFAULTS.CombatPosition.x,  y = DEFAULTS.CombatPosition.y,  z = DEFAULTS.CombatPosition.z },
        LockOnPosition  = { x = DEFAULTS.LockOnPosition.x,  y = DEFAULTS.LockOnPosition.y,  z = DEFAULTS.LockOnPosition.z },
        ...
        SprintPosition  = { x = DEFAULTS.SprintPosition.x,  y = DEFAULTS.SprintPosition.y,  z = DEFAULTS.SprintPosition.z },
...
local function load_file(path, container)
    ...
    local success, err = pcall(function()
        for line in f:lines() do
            ...
            elseif key == "WalkFOV"   then container.fovs.walk   = tonumber(value) or container.fovs.walk
            elseif key == "SprintFOV" then container.fovs.sprint = tonumber(value) or container.fovs.sprint
            elseif key == "LockOnFOV" then container.fovs.lockon = tonumber(value) or container.fovs.lockon
            ...
            elseif key == "DefaultCamZ" then container.DefaultPosition.z = sanitize_number(value)

            elseif key == "CombatCamX" then container.CombatPosition.x = sanitize_number(value)
            elseif key == "CombatCamY" then container.CombatPosition.y = sanitize_number(value)
            elseif key == "CombatCamZ" then container.CombatPosition.z = sanitize_number(value)

            elseif key == "LockOnCamX" then container.LockOnPosition.x = sanitize_number(value)
            elseif key == "LockOnCamY" then container.LockOnPosition.y = sanitize_number(value)
            elseif key == "LockOnCamZ" then container.LockOnPosition.z = sanitize_number(value)
            
            elseif key == "IdleCamX" then container.IdlePosition.x = sanitize_number(value)
            elseif key == "IdleCamY" then container.IdlePosition.y = sanitize_number(value)
            elseif key == "IdleCamZ" then container.IdlePosition.z = sanitize_number(value)

            elseif key == "WalkCamX" then container.WalkPosition.x = sanitize_number(value)
            elseif key == "WalkCamY" then container.WalkPosition.y = sanitize_number(value)
            elseif key == "WalkCamZ" then container.WalkPosition.z = sanitize_number(value)

            elseif key == "SprintCamX" then container.SprintPosition.x = sanitize_number(value)
            elseif key == "SprintCamY" then container.SprintPosition.y = sanitize_number(value)
            elseif key == "SprintCamZ" then container.SprintPosition.z = sanitize_number(value)

            elseif key == "LockOnYawBias" then
                local n = tonumber(value)
                if n then container.LockOnYawBias = n end

            elseif key == "LockOnPitchBias" then
                local n = tonumber(value)
                if n then container.LockOnPitchBias = n end

            elseif key == "DisableCameraCollision" or key == "EnableIdleCamera" or key == "EnableWalkingCamera" or key == "EnableSprintingCamera" or key == "EnableLockOnCamera" then
...
function M.load_preset(num)
    ...
    for k, v in pairs(loaded.fovs) do cfg.fovs[k] = v end
